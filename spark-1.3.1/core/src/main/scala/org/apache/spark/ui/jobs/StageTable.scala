...
private[ui] class StageTableBase(
        ...
        <th>Stage Id</th> ++ {
            if (isFairScheduler) {
                ...
                <th>Description</th>
                        ...
                        <th>
                            <span data-toggle="tooltip" title={ToolTips.OUTPUT}>Output</span>
                        </th>
                        <th>
    ...
    private def makeDescription(s: StageInfo): Seq[Node] = {
        ...
        val nameLink = <a href={nameLinkUri}>
            {s.name}
        </a>
        ...
        val stageDesc = for {
            ...
            <span class="description-input" title={desc}>
                {desc}
            </span>
        }
        <div>
    ...
    protected def stageRow(s: StageInfo): Seq[Node] = {
        ...
        if (stageDataOption.isEmpty) {
            return <td>
                {s.stageId}
            </td> <td>No data available for this stage</td>
        }
            ...
            if (isFairScheduler) {
                ...
                <td>
                    {makeDescription(s)}
                        ...
                        <td sorttable_customkey={inputRead.toString}>
                            {inputReadWithUnit}
                        </td>
                        <td sorttable_customkey={outputWrite.toString}>
                            {outputWriteWithUnit}
                        </td>
                        <td sorttable_customkey={shuffleRead.toString}>
                            {shuffleReadWithUnit}
                        </td>
                        <td sorttable_customkey={shuffleWrite.toString}>
                            {shuffleWriteWithUnit}
                        </td>
    }

    /** Render an HTML row that represents a stage */
    private def renderStageRow(s: StageInfo): Seq[Node] = <tr>
        {stageRow(s)}
    </tr>
}

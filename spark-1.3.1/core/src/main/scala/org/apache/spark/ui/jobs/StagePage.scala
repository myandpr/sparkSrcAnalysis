...
private[ui] class StagePage(parent: StagesTab) extends WebUIPage("stage") {
        ...
        listener.synchronized {
            ...
            val summary =
                <div>
                    <ul class="unstyled">
                        <li>
                            <strong>Total task time across all tasks:</strong>{UIUtils.formatDuration(stageData.executorRunTime)}
                        </li>{if (stageData.hasInput) {
                        <li>
                        ...
                        <li>
                            <strong>Shuffle read:</strong>{s"${Utils.bytesToString(stageData.shuffleReadTotalBytes)} / " +
                    ...
                    <div class="additional-metrics collapsed">
                        <ul style="list-style-type:none">
                            <li>
                                ...
                                <span class="additional-metric-title">
                                    <em>(De)select All</em>
                                </span>
                            </li>
                            <li>
                            ...
                            </li>{if (stageData.hasShuffleRead) {
                            <li>
                                <span data-toggle="tooltip"
            ...
            def accumulableRow(acc: AccumulableInfo) = <tr>
                <td>
                    {acc.name}
                </td> <td>
                    {acc.value}
            ...
            val summaryTable: Option[Seq[Node]] =
                if (validTasks.size == 0) {
                    ...
                    val schedulerDelayTitle = <td>
                        <span data-toggle="tooltip"
                              title={ToolTips.SCHEDULER_DELAY} data-placement="right">Scheduler Delay</span>
                    </td>
                    val schedulerDelayQuantiles = schedulerDelayTitle +:
                            getFormattedTimeQuantiles(schedulerDelays)

                    def getFormattedSizeQuantiles(data: Seq[Double]) =
                        getDistributionQuantiles(data).map(d => <td>
    ...
    def taskRow(
        ...
        taskData match {
            case TaskUIData(info, metrics, errorMessage) =>

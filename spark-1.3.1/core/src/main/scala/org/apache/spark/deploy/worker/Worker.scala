...
/* 首先明确的是Worker不是一个进程，而是一个Actor，通信机制
*  所以会重写actor的三个方法：
* (1)preStart(),处理消息前的准备
* (2)receiveWithLogging(),处理消息，即case操作
* (3)postStop()，处理消息后的善后工作
* */
private[spark] class Worker(
    ...
    // the same time.
    //一共连接16次，间隔时间越来越长，而且具体interval会加一个随机数，保证所有worker请求时间间隔不会共振
    ...
    override def preStart() {
        ...
        /*
        * 此处很重要，
        * registerWithMaster()注册功能->
        * 如果没注册过则tryRegisterAllMasters()向所有master注册->
        * 依次读取master的url对应的actor，像它发actor ! RegisterWorker(workerId, host, port, cores, memory, webUi.boundPort, publicAddress)消息
        * */
        registerWithMaster()
    ...
    //master发生变化，修改通信的master的相关信息，如address等。。。。。
    def changeMaster(url: String, uiUrl: String) {
        // activeMasterUrl it's a valid Spark url since we receive it from master.
        activeMasterUrl = url
        activeMasterWebUiUrl = uiUrl
        master = context.actorSelection(
            //从Master类中获得当前active的master url，拿到该url的actor
    ...
    //向所有master通过master的acorn注册work信息
    private def tryRegisterAllMasters() {
        //masterAkkaUrls是work构造函数本身的传入的参数，worker已知了master的url了
    ...
    /**
      ...
      */
    //重新向master注册，由于网络原因等。。。。或者master死了等，这次如果还不行，worker就give up了，因为没法注册
    ...
    def registerWithMaster() {
        ...
        registrationRetryTimer match {
            //如果没注册，则tryRegisterAllMasters()向所有master注册
            case None =>
                registered = false
                tryRegisterAllMasters()
    ...
    override def receiveWithLogging = {
        //master向其发送已成功注册的Message
        case RegisteredWorker(masterUrl, masterWebUiUrl) =>
            logInfo("Successfully registered with master " + masterUrl)
            registered = true
            //更新worker自己保存的master信息
            changeMaster(masterUrl, masterWebUiUrl)
            //schedule方法是akka控制发消息的的次数和时间
            //Scheduler由ActorSystem提供，可以对Actor的消息发送进行调度。可以通过ActorContext获得当前的ActorSystem，进而获得Scheduler。
            //AKKA支持消息的定期重复执行，方法为schedule()
            /*
            *context.system.scheduler.schedule(2 seconds, 5 seconds, targetActorRef, RequestMessage)
            * 2秒后，间隔5秒重复执行
            * */
            //向master发送心跳消息SendHeartbeat，应该是通过本worker这个actor发送给本worker，然后由本worker通过master的actor发送出去，
            // 因为334行也收到了SendHeartbeat消息
            context.system.scheduler.schedule(0 millis, HEARTBEAT_MILLIS millis, self, SendHeartbeat)
            //清理旧的application工作目录
            if (CLEANUP_ENABLED) {
                logInfo(s"Worker cleanup enabled; old application directories will be deleted in: $workDir")
                //给work这个actor发送清理工作目录的Message
                context.system.scheduler.schedule(CLEANUP_INTERVAL_MILLIS millis,
                    CLEANUP_INTERVAL_MILLIS millis, self, WorkDirCleanup)
            }

        //收到worker发送的sendHeartbeat消息后，通过master这个actor发送给master
        case SendHeartbeat =>
            if (connected) {
        ...
        //接收master发来的master已经改变消息，worker收到后需要重定位master等信息
        ...
        case LaunchExecutor(masterUrl, appId, execId, appDesc, cores_, memory_) =>
            ...
            if (masterUrl != activeMasterUrl) {
                ...
                try {
                    ...
                    // Create the executor's working directory，workDir/appId/execId
                    ...
                    //更新executors变量，val executors = new HashMap[String, ExecutorRunner]
                    /*
                    * key：appId/execId
                    * value:ExecutorRunner
                    * */
                    executors(appId + "/" + execId) = manager

                    /*
                    * ExecutorRunner封装的线程类实例化为manager->
                    * manager.start()->
                    * Thread()->
                    * 调用fetchAndRunExecutor(applicationDescription....)
                    * */
                    manager.start()
                    //更新该worker上的屋里资源量
                    coresUsed += cores_
